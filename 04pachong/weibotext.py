...
#  存放图片的根目录
rootDir = 'F://我的微博//' + weibo_user_name + '正文.html'
# 微博总页数,通过审查元素可知
...
for page, weibo_url in enumerate(weibo_urlList):
    ...
    for entry, weibo_tag in enumerate(weibo_tags_list):
        ...
        if(hms_cct == None):
            continue
        # 获取微博正文部分
        weibo_text = weibo_tag.find('span', attrs={'class': 'ctt'})
        path = rootDir
        with open(path, 'a+', encoding='utf-8_sig') as f:
            f.write(' '.join(hms_cct.text.replace('\xa0', ' ').replace(':', '：').split(' ')[:2]))
            f.write('\r\n')
            f.write('<br />')
            text = weibo_text.text
            if text.endswith('全文'):
                text_a = weibo_tag.find('span', attrs={'class': 'ctt'}).find_all('a')
                if isinstance(text_a,list):
                    text_a = text_a[-1]
                text_r = requests.get('https://weibo.cn' + text_a.attrs['href'], headers=headers, cookies=cookie)
                text = bs(text_r.text, 'html.parser').\
                    find('div', attrs={'class': 'c'}, id=True).\
                    find('span', attrs={'class': 'ctt'}).text
            f.write(text)
            f.write('<br /><br />')
            f.write('\r\n\r\n')
    # time.sleep(0.5)